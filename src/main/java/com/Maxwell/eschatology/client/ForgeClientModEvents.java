...
public class ForgeClientModEvents {    @SubscribeEvent
    ...
    private static final ResourceLocation ICE_OVERLAY =
            new ResourceLocation("eschatology", "textures/gui/ice_overlay.png");    private static float previousProgress = 0.0f;
    private static int fadeInTicks = 0;
    private static final int MAX_FADE_TICKS = 20;     @SubscribeEvent
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null || mc.player.isDeadOrDying()) return;        var effect = mc.player.getEffect(ModEffects.FREEZING_STRIKE.get());
        if (effect == null) {            previousProgress = 0.0f;
            fadeInTicks = 0;
            return;
        }        int amp = effect.getAmplifier();
        float targetProgress = (amp + 1) / 10.0f;        if (fadeInTicks < MAX_FADE_TICKS) {
            fadeInTicks++;
        }        float fadeFactor = (float) fadeInTicks / MAX_FADE_TICKS;
        fadeFactor = Mth.clamp(fadeFactor, 0.0f, 1.0f);        float partial = mc.getFrameTime();
        float progress = Mth.lerp(0.05f * partial, previousProgress, targetProgress);
        previousProgress = progress;        GuiGraphics gui = event.getGuiGraphics();
        int width = mc.getWindow().getGuiScaledWidth();
        int height = mc.getWindow().getGuiScaledHeight();        RenderSystem.disableDepthTest();
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderSystem.setShader(GameRenderer::getPositionTexShader);
        RenderSystem.setShaderTexture(0, ICE_OVERLAY);        float alpha = progress * fadeFactor * 0.4F;
        alpha = Mth.clamp(alpha, 0.05F, 0.45F);
        RenderSystem.setShaderColor(1F, 1F, 1F, alpha);        float border = progress * 0.35f;
        int innerX1 = (int) (width * border);
        int innerY1 = (int) (height * border);
        int innerX2 = (int) (width * (1 - border));
        int innerY2 = (int) (height * (1 - border));        gui.blit(ICE_OVERLAY, 0, 0, 0, 0, width, innerY1, width, height);
        ...
        gui.blit(ICE_OVERLAY, innerX2, innerY1, innerX2, innerY1, width - innerX2, innerY2 - innerY1, width, height);        RenderSystem.disableBlend();
