...
import net.minecraft.world.phys.HitResult;public class SmallBeamEntity extends ThrowableProjectile {
    ...
    public void tick() {
        super.tick();        if (this.level().isClientSide) {            for (int i = 0; i < 3; i++) {
    ...
    protected void onHit(HitResult result) {
        super.onHit(result);        if (this.level().isClientSide) return;        if (result.getType() == HitResult.Type.ENTITY) {
            EntityHitResult entityHitResult = (EntityHitResult) result;
            if (entityHitResult.getEntity() instanceof LivingEntity target) {
                if (target == this.getOwner() || (this.getOwner() != null && this.getOwner().isAlliedTo(target))) {
                    return;
                }                target.invulnerableTime = 0;
                target.hurt(this.damageSources().explosion(this, this.getOwner()), this.damage);                int duration = (int) (ExoWitherBalance.SMALL_BEAM_EFFECT_BASE_DURATION + this.damage * ExoWitherBalance.SMALL_BEAM_EFFECT_DAMAGE_DURATION_MULTIPLIER);
                ...
                target.addEffect(new MobEffectInstance(ModEffects.FREEZING_STRIKE.get(), duration, amplifier));                target.level().playSound(null, target.blockPosition(), SoundEvents.PLAYER_HURT_FREEZE, target.getSoundSource(), 0.7F, 1.2F);
                ((Level) target.level()).addParticle(ParticleTypes.SNOWFLAKE,
                        target.getX(), target.getEyeY(), target.getZ(),
                        0.0D, 0.1D, 0.0D);                if (this.enraged && ++this.pierceCount < ExoWitherBalance.SMALL_BEAM_ENRAGED_PIERCE_COUNT) {
                    return;
                }                this.discard();
            }
        } else if (result.getType() == HitResult.Type.BLOCK) {            this.level().playSound(null, this.blockPosition(), SoundEvents.GLASS_BREAK, this.getSoundSource(), 0.6F, 1.8F);
