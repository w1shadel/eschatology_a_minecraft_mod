...
public class MissileAttackGoal extends Goal {
    ...
    private static final int ANIMATION_DURATION_TICKS = (int) (ExoWitherAnimationTicks.MISSILE_SHOOT_ATTACK_LENGTH_IN_SECONDS * 20);

    public MissileAttackGoal(ExoWither owner, int cooldown) {
        this.owner = owner;
    ...
    public void start() {
        ...
        this.fireInterval = this.isEnraged ?
    ...
    public void tick() {
        ...
        this.attackTime++;
        if (this.attackTime % this.fireInterval == 0 && this.missilesFired < ExoWitherBalance.MISSILE_COUNT_PER_SIDE * 2) {
    ...
    private void fireMissile(boolean isLeft, LivingEntity target) {
        ...
        Vec3 rightVec = this.owner.getLookAngle().cross(new Vec3(0, 1, 0)).normalize();
        double sideOffset = isLeft ? ExoWitherBalance.MISSILE_SPAWN_SIDE_OFFSET : -ExoWitherBalance.MISSILE_SPAWN_SIDE_OFFSET;
        double randomOffset = ExoWitherBalance.MISSILE_SPAWN_RANDOM_OFFSET;
        Vec3 spawnPos = this.owner.getEyePosition()
                ...
                .add((this.owner.getRandom().nextDouble() - 0.5) * randomOffset,
                        ...
                        (this.owner.getRandom().nextDouble() - 0.5) * randomOffset);
        ExoMissile missile = new ExoMissile(this.owner.level(), this.owner, target);
