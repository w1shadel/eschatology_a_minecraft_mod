...
import net.minecraftforge.network.NetworkHooks;import java.util.List;public class FrostFieldEntity extends Entity {
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            for (int i = 0; i < 2; ++i) {
                float angle = this.random.nextFloat() * 2.0F * (float) Math.PI;
            ...
            if (this.tickCount % ExoWitherBalance.FROST_FIELD_EFFECT_INTERVAL == 0) {
                ...
                for (LivingEntity entity : nearbyEntities) {
                    ...
                    var existing = entity.getEffect(ModEffects.FREEZING_STRIKE.get());
                    int newLevel = 0;
                    int newDuration = ExoWitherBalance.FROST_FIELD_EFFECT_BASE_DURATION;
                    if (existing != null) {
