package com.Maxwell.eschatology.Boss.XF07_Revanant.Entities.ExoMisslie;
...
public class ExoMissile extends Projectile {
    ...
    public void tick() {
        super.tick();        if (level().isClientSide) {
            ...
            setDeltaMovement(newVel);            float yRot = (float) (Math.atan2(newVel.x, newVel.z) * (180F / Math.PI));
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (level().isClientSide) return;        Entity hit = result.getEntity();
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        if (level().isClientSide) return;        ServerLevel server = (ServerLevel) level();        server.sendParticles(ParticleTypes.EXPLOSION_EMITTER, pos.x, pos.y, pos.z, 1, 0.5, 0.5, 0.5, 0);
        server.sendParticles(ParticleTypes.SNOWFLAKE, pos.x, pos.y + 0.5, pos.z, 20, 0.6, 0.6, 0.6, 0.02);
        server.playSound(null, pos.x, pos.y, pos.z, SoundEvents.GENERIC_EXPLODE, getSoundSource(), 1.2F, 0.8F + random.nextFloat() * 0.4F);        float blastDamage = damage * ExoWitherBalance.EXO_MISSILE_EXPLOSION_DAMAGE_MULTIPLIER;
        AABB box = new AABB(
        ...
        for (LivingEntity e : entities) {
            if (e != shooter) {
                e.hurt(damageSources().explosion(this, null), blastDamage);
            }
        }        FrostFieldEntity field = new FrostFieldEntity(server, pos.x, pos.y, pos.z);
        if (shooter != null) field.setOwnerUUID(shooter.getUUID());
        server.addFreshEntity(field);        if (!splitDone) {
            splitDone = true;
            spawnSplitMissiles(pos);
        }        discard();
    }    private void spawnSplitMissiles(Vec3 origin) {
        if (level().isClientSide) return;
        ServerLevel server = (ServerLevel) level();        for (int i = 0; i < ExoWitherBalance.EXO_MISSILE_SPLIT_COUNT; i++) {
            ...
            Vec3 spread = baseVel.yRot((float) yawOffset).add(0, ExoWitherBalance.EXO_MISSILE_SPLIT_VERTICAL_OFFSET, 0).normalize().scale(ExoWitherBalance.EXO_MISSILE_SPLIT_SPEED_SCALE);            ExoMissile child = new ExoMissile(ModEntities.EXO_MISSILE.get(), level());
    ...
    protected void readAdditionalSaveData(net.minecraft.nbt.CompoundTag tag) {}    @Override
    protected void addAdditionalSaveData(net.minecraft.nbt.CompoundTag tag) {}    @Override
