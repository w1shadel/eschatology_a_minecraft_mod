...
public class MortarBarrageGoal extends Goal {
    ...
    private static final int TOTAL_DURATION = ExoWitherBalance.MORTAR_PREPARE_DURATION_TICKS +
            (ExoWitherBalance.MORTAR_FIRE_INTERVAL_TICKS * ExoWitherBalance.MORTAR_TOTAL_SHOTS);

    public MortarBarrageGoal(ExoWither owner, int cooldown) {
        this.owner = owner;
    ...
    private void executeBarrage() {
        if (this.targetPos == null || !(this.owner.level() instanceof ServerLevel level)) return;
        double radius = ExoWitherBalance.MORTAR_IMPACT_RANDOM_RADIUS;
        ...
        Vec3 impactPos = new Vec3(this.targetPos.x() + offsetX, this.targetPos.y(), this.targetPos.z() + offsetZ);
        level.sendParticles(ParticleTypes.EXPLOSION_EMITTER, impactPos.x, impactPos.y, impactPos.z, 2, 1.0, 1.0, 1.0, 0.0);
        level.playSound(null, impactPos.x, impactPos.y, impactPos.z, SoundEvents.GENERIC_EXPLODE, this.owner.getSoundSource(), 1.0F, 1.5F);
        float damage = (float) this.owner.getAttributeValue(Attributes.ATTACK_DAMAGE) * ExoWitherBalance.MORTAR_DAMAGE_RATIO;
        ...
        for (LivingEntity entity : entities) {
            if (entity != this.owner && !this.owner.isAlliedTo(entity)) {
                entity.hurt(this.owner.damageSources().outOfBorder(), damage);
            }
        }
    }
}