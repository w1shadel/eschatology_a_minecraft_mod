    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            ...
            if (this.tickCount % 5 == 0) {
                ...
                AABB damageArea = this.getBoundingBox().inflate(this.radius, 1.0, this.radius);
                ...
                for (LivingEntity entity : entities) {
                    if (entity != currentOwner && !entity.isAlliedTo(currentOwner)) {
                        float damage = (float) currentOwner.getAttributeValue(Attributes.ATTACK_DAMAGE) * 0.15f;
                        DamageSource damageSource = this.damageSources().outOfBorder();
                        entity.hurt(damageSource, damage);
                        entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 10, 3, true, false));
                        ((ServerLevel) this.level()).sendParticles(ParticleTypes.DAMAGE_INDICATOR,
                                entity.getX(),
                                entity.getY(0.5),
                                entity.getZ(),
                                5,
                                0.2, 0.2, 0.2, 0.0);
                        this.level().playSound(null, entity.blockPosition(), SoundEvents.FIRE_EXTINGUISH, entity.getSoundSource(), 0.5F, 1.5F);
                    }
                }
            }
        }
