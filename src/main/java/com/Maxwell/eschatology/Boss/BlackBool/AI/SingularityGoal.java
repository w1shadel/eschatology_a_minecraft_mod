...
import java.util.List;public class SingularityGoal extends Goal {
    ...
    private static final int DURATION_CHARGE_UP = 30;
    private static final int DURATION_PULLING = 70;
    private static final float RADIUS_PULL = 8.0f;
    private static final float RADIUS_EXPLOSION = 5.0f;

    private enum Phase {INACTIVE, CHARGING, PULLING, COLLAPSING}

    private Phase currentPhase = Phase.INACTIVE;

    public SingularityGoal(BlackBool mob) {
        this.mob = mob;
    ...
    public boolean canContinueToUse() {
        return this.currentPhase != Phase.INACTIVE && this.mob.getTarget() != null && this.mob.getTarget().isAlive();
    }
    ...
    public void start() {
        ...
        this.mob.singularityCooldown = this.mob.getCooldownTicksByHealth(400);
        this.targetPos = this.mob.getTarget().blockPosition();
        this.mob.playSound(SoundEvents.ENDERMAN_STARE, 2.0F, 0.8F);
    }
        ...
        switch (this.currentPhase) {
            case CHARGING:
                if (this.mob.level() instanceof ServerLevel serverLevel) {
                    double angle = (this.ticks * 15) * (Math.PI / 180D);
                    double radius = 2.0;
            ...
            case PULLING:
                if (this.mob.level() instanceof ServerLevel serverLevel) {
                    ...
                    List<Player> playersToPull = this.mob.level().getEntitiesOfClass(Player.class, pullBox);
                    for (Player player : playersToPull) {
                        Vec3 centerPos = Vec3.atCenterOf(this.targetPos);
                        Vec3 pullDir = centerPos.subtract(player.position()).normalize();
                        double pullStrength = 0.05 + (0.3 * ((double) this.ticks / DURATION_PULLING));
                        player.addDeltaMovement(pullDir.scale(pullStrength));
                        player.fallDistance = 0;
                    }
                    RandomSource random = RandomSource.create(this.mob.getId());
                    if (this.ticks % 2 == 0) {
                ...
                if (this.ticks > 20 && this.ticks % 30 == 0) {
                    if (!this.mob.level().isClientSide) {
                        this.mob.playSound(SoundEvents.PLAYER_ATTACK_SWEEP, 1.5F, 0.5F);
                        ...
                        for (Vec3 dir : directions) {
                            Vec3 waveVelocity = dir.normalize().scale(0.7D);
                            VoidWaveEntity wave = new VoidWaveEntity(this.mob.level(), this.mob, waveVelocity);
                            wave.setPos(centerPos);
                            this.mob.level().addFreshEntity(wave);
                        }
                    }
                }
            ...
            case COLLAPSING:
                if (this.mob.level() instanceof ServerLevel serverLevel) {
                    ...
                    serverLevel.sendParticles(ParticleTypes.EXPLOSION_EMITTER, this.targetPos.getX() + 0.5, this.targetPos.getY() + 0.5, this.targetPos.getZ() + 0.5, 2, 0.0, 0.0, 0.0, 1.0);
                    AABB explosionBox = new AABB(this.targetPos).inflate(RADIUS_EXPLOSION);
                    List<Player> playersToDamage = this.mob.level().getEntitiesOfClass(Player.class, explosionBox);
