...
public class EventHorizonControllerRenderer extends EntityRenderer<EventHorizonControllerEntity> {
    public EventHorizonControllerRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }
    ...
    public void render(EventHorizonControllerEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        pPoseStack.mulPose(Axis.YP.rotationDegrees(ageInTicks * 0.2f));
        VertexConsumer vertexConsumer = pBuffer.getBuffer(CustomRenderTypes.NEBULA_TYPE);
        Matrix4f matrix = pPoseStack.last().pose();
        int segments = 360;
        float textureRepeatX = 16.0f;
        float x1 = (float) radius;
        float z1 = 0;
        float u1 = 0;
        for (int i = 0; i < segments; i++) {
            float nextAngle = (float) (i + 1) / segments * (float) (2 * Math.PI);
            float x2 = (float) (Math.cos(nextAngle) * radius);
            float z2 = (float) (Math.sin(nextAngle) * radius);
            float u2 = ((float) (i + 1) / segments) * textureRepeatX;
            float v0 = -(ageInTicks * 0.005f);
            float v1 = (float) (height / 64.0f) + v0;
            float nX1 = x1 / (float) radius;
            float nZ1 = z1 / (float) radius;
            float nX2 = x2 / (float) radius;
            float nZ2 = z2 / (float) radius;
            vertex(vertexConsumer, matrix, x1, 0, z1, u1, v1, nX1, nZ1, pPackedLight);
            ...
            vertex(vertexConsumer, matrix, x2, 0, z2, u2, v1, nX2, nZ2, pPackedLight);
            x1 = x2;
            z1 = z2;
            u1 = u2;
        }
