...
public class VoidRiftRenderer extends EntityRenderer<VoidRiftEntity> {
    private static final ResourceLocation RIFT_TEXTURE = new ResourceLocation(Eschatology.MODID, "textures/entity/void_rift.png");

    public VoidRiftRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }
    ...
    public void render(VoidRiftEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        float uvScrollSpeed = -totalTicks * 0.03F;
        ...
        drawLaserPlane(vertexConsumer, matrix4f, pPackedLight, height, width, uvScrollSpeed);
    ...
    private void drawLaserPlane(VertexConsumer pConsumer, Matrix4f pMatrix, int pPackedLight, float height, float width, float vOffset) {
        vertex(pConsumer, pMatrix, pPackedLight, -width / 2, height, 0.0F, 1.0F + vOffset, 180, 200, 255, 150);
