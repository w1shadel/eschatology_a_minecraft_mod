...
public class EndLaserBeamDMGFieldEntity extends Entity {    private static final EntityDataAccessor<Integer> DATA_OWNER_ID =
            SynchedEntityData.defineId(EndLaserBeamDMGFieldEntity.class, EntityDataSerializers.INT);    private LivingEntity owner;    private UUID ownerUUID;    private static final int DURATION = 80;
    ...
    protected void defineSynchedData() {        this.entityData.define(DATA_OWNER_ID, 0);
    }    @Override
    public void tick() {
        super.tick();        if (this.tickCount > DURATION) {
            this.discard();
            return;
            ...
            if (this.tickCount == 1) { 
                ...
                if (currentOwner == null) return;                 AABB damageArea = new AABB(
                        this.getX() - RADIUS, this.getY(), this.getZ() - RADIUS,
                        this.getX() + RADIUS, this.getY() + HEIGHT, this.getZ() + RADIUS
                );
                List<LivingEntity> entities = this.level().getEntitiesOfClass(LivingEntity.class, damageArea);                for (LivingEntity entity : entities) {                    if (!entity.is(currentOwner) && !entity.isAlliedTo(currentOwner)) {
                        entity.invulnerableTime = 0;                         entity.hurt(this.damageSources().indirectMagic(this, currentOwner), 4.0f);
                    }
                }
            }
        }
    ...
    public LivingEntity getOwner() {
        ...
        if (this.level().isClientSide) {            Entity entity = this.level().getEntity(this.entityData.get(DATA_OWNER_ID));
            if (entity instanceof LivingEntity) {
                this.owner = (LivingEntity) entity;
                return this.owner;
            }
        } else if (this.ownerUUID != null) {            Entity entity = ((ServerLevel)this.level()).getEntity(this.ownerUUID);
            if (entity instanceof LivingEntity) {
                this.owner = (LivingEntity) entity;
                return this.owner;
            }
