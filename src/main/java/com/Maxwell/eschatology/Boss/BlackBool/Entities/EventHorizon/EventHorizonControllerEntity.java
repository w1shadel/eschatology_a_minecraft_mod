...
public class EventHorizonControllerEntity extends Entity {
    ...
    private static final double ARENA_RADIUS = 64.0;
    private int orbCooldown = 0;

    public EventHorizonControllerEntity(EntityType<?> pEntityType, Level pLevel) {
        ...
        if (this.level().isClientSide) {
            if (this.tickCount % 2 == 0) {
                for (int i = 0; i < 360; i += 5) {
                    ...
                    BlockPos groundPos = new BlockPos((int) x, (int) this.getY(), (int) z);
            ...
            for (Player player : players) {
                ...
                double distanceSqFromCenter = player.distanceToSqr(this.position());
                if (distanceSqFromCenter > ARENA_RADIUS * ARENA_RADIUS) {
    ...
    private void spawnOrbFromBoundaryTo(LivingEntity target) {
        ...
        double x = this.getX() + ARENA_RADIUS * Math.cos(angle);
        double z = this.getZ() + ARENA_RADIUS * Math.sin(angle);
        Vec3 spawnPos = new Vec3(x, target.getY() + 2.0, z);
        LightOrbEntity orb = new LightOrbEntity(this.level(), owner, target);
        orb.setPos(spawnPos);
