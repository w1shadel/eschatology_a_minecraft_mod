...
public class BlackBoolRenderer extends MobRenderer<BlackBool, BlackBoolModel> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(Eschatology.MODID, "textures/entity/blackbole.png");
    private static final ResourceLocation RING_TEXTURE = new ResourceLocation(Eschatology.MODID, "textures/entity/ring.png");
    private static final ResourceLocation AURA_TEXTURE = new ResourceLocation(Eschatology.MODID, "textures/entity/effect/aura.png");
    private static final Vector3f COLOR_NORMAL = new Vector3f(0.0F, 0.0F, 0.0F);
    ...
    private static final Vector3f COLOR_PHASE_TWO = new Vector3f(0.6F, 0.0F, 1.0F);

    public BlackBoolRenderer(EntityRendererProvider.Context renderManagerIn) {
    ...
    @Override
    public void render(BlackBool pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        if (pEntity.getDeathAnimationTicks() > 0) {
            renderDeathSequence(pEntity, pPartialTicks, pPoseStack, pBuffer, pPackedLight);
        } else {
    ...
    private void drawSuctionDebris(BlackBool entity, PoseStack poseStack, VertexConsumer consumer, int packedLight, float ageInTicks, int index, float distance, Vector3f color) {
        ...
        poseStack.mulPose(Axis.YP.rotationDegrees(-yaw));
        poseStack.mulPose(Axis.XP.rotationDegrees(-pitch));
        poseStack.mulPose(Axis.YP.rotationDegrees(ageInTicks * random.nextFloat() * 5.0F));
        poseStack.mulPose(Axis.XP.rotationDegrees(ageInTicks * random.nextFloat() * 5.0F));
        drawThickLineShape(poseStack, consumer, packedLight, 3, 0.2F + random.nextFloat() * 0.2F, 0.03F, color);
        poseStack.popPose();
    }
    ...
    private void drawSpreadingDebris(BlackBool entity, PoseStack poseStack, VertexConsumer consumer, int packedLight, float ageInTicks, int index, float distance, Vector3f color) {
        ...
        poseStack.mulPose(Axis.XP.rotationDegrees(pitch));
        poseStack.translate(distance, 0, 0);
        poseStack.mulPose(Axis.YP.rotationDegrees(ageInTicks * (1.0f + random.nextFloat())));
        poseStack.mulPose(Axis.XP.rotationDegrees(ageInTicks * (1.0f + random.nextFloat())));
        drawThickLineShape(poseStack, consumer, packedLight, 4, 0.15F, 0.015F, color);
        poseStack.popPose();
    }

    private void renderLightSphere(PoseStack poseStack, MultiBufferSource buffer, float progress) {
        float intensity = Mth.sin(progress * Mth.PI);
        float scale = Mth.lerp(progress, 0.1F, 8.0F);
        float alpha = intensity * 0.8F;
        if (alpha <= 0.0F) return;
        poseStack.pushPose();
        poseStack.scale(scale, scale, scale);
        VertexConsumer consumer = buffer.getBuffer(RenderType.energySwirl(AURA_TEXTURE, progress * 2.0F % 1.0F, progress * 2.0F % 1.0F));
        Matrix4f matrix = poseStack.last().pose();
        int color = ((int) (alpha * 255) << 24) | 0xAA88FF;
        for (int i = 0; i < 3; i++) {
            poseStack.pushPose();
    ...
    private void renderDeathSequence(BlackBool pEntity, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        float ageInTicks = pEntity.tickCount + pPartialTicks;
        final int OUTBURST_END_TICK = 40;
        final int COLLAPSE_END_TICK = OUTBURST_END_TICK + 80;
        final int IMPLOSION_END_TICK = COLLAPSE_END_TICK + 60;
        if (deathTicks > OUTBURST_END_TICK && deathTicks <= COLLAPSE_END_TICK) {
            float collapseProgress = (deathTicks - OUTBURST_END_TICK) / (float) (COLLAPSE_END_TICK - OUTBURST_END_TICK);
            float scale = Mth.lerp(collapseProgress, 1.0F, 0.0F);
            pPoseStack.pushPose();
        ...
        if (deathTicks <= OUTBURST_END_TICK) {
            ...
            for (int i = 0; i < 15; i++) {
                pPoseStack.pushPose();
                pPoseStack.mulPose(Axis.YP.rotationDegrees(random.nextFloat() * 360));
                pPoseStack.mulPose(Axis.XP.rotationDegrees(random.nextFloat() * 180 - 90));
                float length = 1.0F + random.nextFloat() * (deathTicks / (float) OUTBURST_END_TICK) * 3.0F;
                pPoseStack.scale(length, length, length);
                drawThickLine(pPoseStack, consumer, pPackedLight, new Vector3f(0, 0, 0), new Vector3f(1, 0, 0), 0.02F, COLOR_ENRAGED);
                pPoseStack.popPose();
            }
            ...
            VertexConsumer consumer = pBuffer.getBuffer(CustomRenderTypes.DEBRIS_TYPE);
            for (int i = 0; i < 100; i++) {
                float initialDistance = 15.0F;
                float currentDistance = Mth.lerp(collapseProgress, initialDistance, 0.0F);
                drawSuctionDebris(pEntity, pPoseStack, consumer, pPackedLight, ageInTicks, i, currentDistance, COLOR_NORMAL);
            }
            ...
            VertexConsumer consumer = pBuffer.getBuffer(CustomRenderTypes.DEBRIS_TYPE);
            renderLightSphere(pPoseStack, pBuffer, implosionProgress);
            for (int i = 0; i < 150; i++) {
                float currentDistance = Mth.lerp(implosionProgress, 0.0F, 20.0F);
                Vector3f cosmicColor = new Vector3f(0.6f, 0.8f, 1.0f);
                drawSpreadingDebris(pEntity, pPoseStack, consumer, pPackedLight, ageInTicks, i, currentDistance, cosmicColor);
            }
        }
        pPoseStack.popPose();
    }
    ...
    private void renderNormalEffects(BlackBool pEntity, float pAgeInTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        pPoseStack.popPose();
        float healthPercent = pEntity.getHealthPercent();
        ...
        VertexConsumer consumer = pBuffer.getBuffer(CustomRenderTypes.DEBRIS_TYPE);
        int particleCount = determineParticleCount(healthPercent, isPhaseTwo);
    ...
    private void drawThickLine(PoseStack poseStack, VertexConsumer consumer, int packedLight, Vector3f start, Vector3f end, float thickness, Vector3f color) {
        ...
        Vector3f thicknessDir = new Vector3f(lineDir).cross(new Vector3f(0, 1, 0)).normalize();
        if (thicknessDir.lengthSquared() < 0.1f) {
            thicknessDir = new Vector3f(lineDir).cross(new Vector3f(0, 0, 1)).normalize();
        }
        thicknessDir.mul(thickness / 2.0f);
        Vector3f v1 = new Vector3f(start).add(thicknessDir);
        Vector3f v2 = new Vector3f(start).sub(thicknessDir);
        Vector3f v3 = new Vector3f(end).sub(thicknessDir);
        Vector3f v4 = new Vector3f(end).add(thicknessDir);
        float r = color.x(), g = color.y(), b = color.z(), a = 0.7f;
        Vector3f normal = new Vector3f(end).sub(start).cross(thicknessDir).normalize();
        ...
        consumer.vertex(matrix, v3.x(), v3.y(), v3.z()).color(r, g, b, a).uv(1, 1).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(normalMatrix, normal.x(), normal.y(), normal.z()).endVertex();
        consumer.vertex(matrix, v3.x(), v3.y(), v3.z()).color(r, g, b, a).uv(1, 1).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(normalMatrix, normal.x(), normal.y(), normal.z()).endVertex();
