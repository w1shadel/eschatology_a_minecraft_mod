...
public class SingularityEntity extends Entity {
    ...
    public SingularityEntity(EntityType<? extends SingularityEntity> pEntityType, Level pLevel) {
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            float ageInTicks = this.tickCount;
            for (int i = 0; i < 5; i++) {
            ...
            for (LivingEntity entity : entities) {
                if (entity != currentOwner && (currentOwner == null || !entity.isAlliedTo(currentOwner))) {
        ...
        if (!this.level().isClientSide) {
            ...
            for (LivingEntity entity : entities) {
                if (entity != currentOwner && !entity.isAlliedTo(currentOwner)) {
                    float damage = (float) currentOwner.getAttributeValue(Attributes.ATTACK_DAMAGE);
                    entity.hurt(this.damageSources().indirectMagic(this, currentOwner), damage);
                }
            }
        }
    }
