...
public class TOaFGTickHandler {

    private static final int TARGET_RANGE = 16;

    private static final int PARTICLES_PER_TARGET = 10; 
    ...
    private static void spawnParticlesAroundTarget(ServerLevel serverLevel, Monster target) {
        ...
        final int TRAIL_PARTICLE_COUNT = 4;
        ...
        for (int i = 0; i < PARTICLES_PER_TARGET; i++) {
            ...
            Vec3 startPos = targetPos.add(offsetX, offsetY, offsetZ);


            Vec3 travelVector = targetPos.subtract(startPos);

            for (int j = 1; j <= TRAIL_PARTICLE_COUNT; j++) {

                Vec3 trailPos = startPos.add(travelVector.scale((float)j / (TRAIL_PARTICLE_COUNT + 1)));
