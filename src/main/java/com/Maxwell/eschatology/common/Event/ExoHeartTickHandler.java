...
import top.theillusivec4.curios.api.CuriosApi;import java.util.WeakHashMap;
...
public class ExoHeartTickHandler {
    private static final TagKey<EntityType<?>> BOSS_TAG =
            TagKey.create(ForgeRegistries.ENTITY_TYPES.getRegistryKey(), new ResourceLocation("forge", "bosses"));    private static final WeakHashMap<Player, Integer> lastNearBossTick = new WeakHashMap<>();    @SubscribeEvent
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (level.isClientSide || player.isSpectator() || player.isCreative()) return;        player.getCapability(ExoHeartData.CAPABILITY).ifPresent(data -> {
            ...
            ).isEmpty();            if (data.isActive() && hasHeart) {
            ...
            }            charge = Math.max(0, Math.min(ModConstants.ExoHeart.MAX_CHARGE, charge));            if (charge != before) {
