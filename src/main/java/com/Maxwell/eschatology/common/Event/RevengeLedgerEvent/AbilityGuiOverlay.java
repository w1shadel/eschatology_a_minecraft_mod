...
public class AbilityGuiOverlay {
    private static final Queue<Dialogue> dialogueQueue = new LinkedList<>();
    ...
    private static class Dialogue {
        ...
        Dialogue(String key, int durationTicks) {
    ...
    public static void onRenderGuiPost(RenderGuiOverlayEvent.Post event) {
        ...
        Component textToRender = currentDialogue.text;
        if (textToRender == null) return;
        float scale = 1.5f;
        float verticalRatio = 0.75f;
        float alpha = currentDialogue.getAlpha();
        if (alpha <= 0.01f) {
