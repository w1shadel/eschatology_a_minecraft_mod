...
public class RevengeGaugeOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (!ClientConfig.CLIENT.showRevengeGauge.get()) return;
        ...
        int configX = ClientConfig.CLIENT.revengeGaugeX.get();
        int configYOffset = ClientConfig.CLIENT.revengeGaugeYOffset.get();
        int width = ClientConfig.CLIENT.revengeGaugeWidth.get();
        int height = ClientConfig.CLIENT.revengeGaugeHeight.get();

        int x = configX;

        int y = mc.getWindow().getGuiScaledHeight() / 2 + configYOffset;
