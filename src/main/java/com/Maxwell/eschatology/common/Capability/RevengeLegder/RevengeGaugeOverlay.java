...
public class RevengeGaugeOverlay {    private static final ResourceLocation GAUGE_BG = new ResourceLocation("eschatology", "textures/gui/revenge_gauge_bg.png");
    private static final ResourceLocation GAUGE_FILL = new ResourceLocation("eschatology", "textures/gui/revenge_gauge_fill.png");    @SubscribeEvent
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) return;
        if (mc.options.hideGui) return;
        if (!mc.player.getMainHandItem().is(ModItems.REVENGE_LEDGER.get()) &&
        ...
        gg.fill(x, y, x + width, y + height, 0x80000000);
        int filledHeight = (int) (height * ratio);
        ...
        gg.fill(x + 1, filledY + 1, x + width - 1, y + height - 1, color);
        String percent = (int) (ratio * 100) + "%";
        gg.drawString(mc.font, percent, x + width + 5, y + height / 2, 0xFFFFFF);
    }
}