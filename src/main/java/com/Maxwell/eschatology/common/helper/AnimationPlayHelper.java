...
public class AnimationPlayHelper {
    ...
    private static class AnimationState {
        ...
        AnimationState(KeyframeAnimation anim, boolean damage, boolean lock) {
    ...
    public static void playAnimation(Player player, String animationName, boolean damage, boolean lock) {

        Minecraft mc = Minecraft.getInstance();

        var data = PlayerAnimationAccess.getPlayerAssociatedData((AbstractClientPlayer) player);
        ModifierLayer<IAnimation> layer = (ModifierLayer<IAnimation>) data.get(ModAnimations.ANIMATION_LAYER_ID);
        if (layer == null) return;

        ResourceLocation id = new ResourceLocation(Eschatology.MODID, animationName);
        ...
        if (player == mc.player) {
            ...
            localPlayerAnimationState = new AnimationState(anim, damage, lock);
    ...
    public static void onMovementInput(MovementInputUpdateEvent event) {
        ...
        if (localPlayerAnimationState != null && localPlayerAnimationState.lockMovement && localPlayerAnimationState.perspectiveTicks > 0) {
