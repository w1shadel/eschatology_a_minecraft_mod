...
import net.minecraftforge.fml.common.Mod;import java.util.HashMap;
import java.util.Map;
import java.util.UUID;@Mod.EventBusSubscriber(modid = Eschatology.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class CounterStanceHandler {
    ...
    public static void initiateCounterSequence(ServerPlayer player) {        if (!PENDING_COUNTERS.containsKey(player.getUUID()) && !ACTIVE_COUNTERS.containsKey(player.getUUID())) {
            PENDING_COUNTERS.put(player.getUUID(), 0); 
        }
    }    @SubscribeEvent
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {            PENDING_COUNTERS.entrySet().removeIf(entry -> {
                UUID playerUUID = entry.getKey();
                int ticksElapsed = entry.getValue();
                ServerPlayer player = event.getServer().getPlayerList().getPlayer(playerUUID);                if (player == null || !player.isAlive()) {
                    return true; 
                }
                player.setInvulnerable(true);
                ticksElapsed++;                if (ticksElapsed == ModConstants.Counter.SHARD_SPAWN_TICK) {
                ...
                CounterData data = entry.getValue();                if (player == null || !player.isAlive()) {
                    return true;
        ...
        world.playSound(null, centerTarget.x, centerTarget.y, centerTarget.z, SoundEvents.GENERIC_EXPLODE, SoundSource.NEUTRAL, 1.0f, 0.5f);        for (int i = 0; i < 42; i++) {
            ...
            Vec3 spawnPos = centerTarget.add(randomDirection.scale(4.0));            FellBulletShardEntity shard = new FellBulletShardEntity(ModEntities.FELL_SHARD.get(), world, player);
    ...
    }    private static void performAreaDamage(Player player) {        DamageSource damageSource = player.damageSources().magic();
