package com.Maxwell.eschatology.common.Items.FellBullet.Entity;import com.Maxwell.eschatology.common.Items.Awe_Contempt.Event.CounterStanceHandler;
...
import net.minecraft.world.phys.*;public class FellBulletEntity extends Projectile {    private float baseDamage = ModConstants.FellBullet.BASE_DAMAGE;    public FellBulletEntity(EntityType<? extends Projectile> type, Level level) {
        super(type, level);
    ...
    @Override
    public void tick() {
        super.tick();        Vec3 currentPos = this.position();
        Vec3 nextPos = currentPos.add(this.getDeltaMovement());        BlockHitResult blockHitResult = this.level().clip(new ClipContext(currentPos, nextPos, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, this));        if (blockHitResult.getType() != HitResult.Type.MISS) {
            nextPos = blockHitResult.getLocation();
        }        AABB searchArea = this.getBoundingBox().expandTowards(this.getDeltaMovement()).inflate(1.0D);
        EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(this.level(), this, currentPos, nextPos, searchArea, this::canHitEntity);        if (entityHitResult != null) {            if (currentPos.distanceToSqr(entityHitResult.getLocation()) < currentPos.distanceToSqr(nextPos)) {                blockHitResult = null; 
            }
        }        HitResult finalHitResult = entityHitResult != null ? entityHitResult : blockHitResult;        if (finalHitResult != null && finalHitResult.getType() != HitResult.Type.MISS) {            if (finalHitResult.getType() == HitResult.Type.ENTITY) {
            ...
            return; 
        }        this.setPos(nextPos);         if (this.level().isClientSide) {
            for (int i = 0; i < 3; ++i) {
    ...
    @Override
    protected boolean canHitEntity(Entity entity) {        if (entity.is(this) || entity.is(this.getOwner())) {
            return false;
    ...
    protected void onHit(HitResult hitResult) {
        ...
        if (world.isClientSide) return;         Vec3 centerTarget = deadEntity.position().add(0, deadEntity.getEyeHeight() / 2.0, 0);
