package com.Maxwell.eschatology.common.Items.Awe_Contempt;import com.Maxwell.eschatology.common.Network.ModMessages;
...
import java.util.List;public class Awe_contempt extends SwordItem {
    public Awe_contempt(Tier pTier, int pAttackDamageModifier, float pAttackSpeedModifier, Properties pProperties) {
        super(pTier, pAttackDamageModifier, pAttackSpeedModifier, pProperties);
    }
    @Override
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {        if (!attacker.level().isClientSide) {            MobEffectInstance inst = target.getEffect(ModEffects.GAZE.get());
            int level = inst == null ? 0 : inst.getAmplifier() + 1;            target.addEffect(new MobEffectInstance(ModEffects.GAZE.get(), 200, level));            if (level >= 7) {
