...
public class RevengeLedgerItem extends SwordItem {    public RevengeLedgerItem(Tier tier, int attackDamageModifier, float attackSpeedModifier, Properties properties) {
        super(tier, attackDamageModifier, attackSpeedModifier, properties);
    }    @Override
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {        if (attacker instanceof Player player && !player.level().isClientSide && target.getPersistentData().getBoolean("RevengeMarked")) {            int gaugeBeforeHit = RevengeData.getGauge(player);
            ...
            }            RevengeData.addGauge(player, 10);            ModMessages.sendToPlayer(new SyncRevengeGaugePacket(RevengeData.getGauge(player)), (ServerPlayer) player);            target.getPersistentData().remove("RevengeMarked");
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide && player instanceof ServerPlayer serverPlayer) {
            if (RevengeData.isMaxGauge(player) && !player.getPersistentData().getBoolean("revenge_skill_active")) {
                ...
                AABB searchBox = new AABB(startPos, startPos.add(lookVec.scale(5))).inflate(2.5);
        ...
        final double BASE_EXTRA_AT_FULL = 8.0D;
        ...
        final double BASE_ATTACK_SPEED = 4.0D;
        double speedFactor = attackSpeed / BASE_ATTACK_SPEED;
        speedFactor = Math.max(0.5D, Math.min(2.0D, speedFactor));
        return BASE_EXTRA_AT_FULL * ratio * speedFactor;
    }
