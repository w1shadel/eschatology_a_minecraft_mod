...
public class WitchsSorn extends Entity {
    @Nullable
    ...
    public WitchsSorn(EntityType<?> pEntityType, Level pLevel) {
        ...
        if (!this.level().isClientSide()) {
            for (LivingEntity livingentity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox())) {
                if (livingentity != this.getOwner() && livingentity.isAlive()) {
                    DamageSource source = this.level().damageSources().indirectMagic(this, this.getOwner());
