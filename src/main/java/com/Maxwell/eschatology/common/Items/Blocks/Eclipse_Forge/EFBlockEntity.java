...
public class EFBlockEntity extends BlockEntity implements MenuProvider {
    ...
    private int currentProcessTime = 80;
    private ItemStack animLeft = ItemStack.EMPTY;
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, EFBlockEntity be) {
        if (!level.isClientSide) {
            if (be.isAnimating && be.currentRecipe == null) {
                be.isAnimating = false;
                be.animationTicks = 0;
            }
            if (be.isAnimating) {
                be.animationTicks++;
                if (be.animationTicks >= be.currentProcessTime) {
                    be.finishCraft();
                    be.isAnimating = false;
                    be.animationTicks = 0;
    ...
    public void startAnimation() {
        if (isAnimating || level == null || level.isClientSide) return;
        ItemStack left = itemHandler.getStackInSlot(SLOT_LEFT);
    ...
    public ItemStack getAnimRight() {
        return animRight;
    }

    public ItemStack getAnimLeft() {
        return animLeft;
    }
}